...
export function BillingView({ subscription, plans, productCount }: BillingViewProps) {
  ...
  const handleDownloadReceipt = (invoice: MerchantSubscription["invoices"][number]) => {
    const lines = [
      "ShopSaaS — Payment Receipt",
      "================================",
      `Invoice:  ${invoice.id}`,
      `Date:     ${formatDate(invoice.date)}`,
      `Plan:     ${invoice.plan}`,
      `Status:   ${invoice.status}`,
      `Amount:   ${formatYen(invoice.amount)}`,
      "================================",
    ...
    toast({ title: "Receipt downloaded", description: `Receipt for ${invoice.id} saved.` })
  }
