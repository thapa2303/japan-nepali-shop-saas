...
import { updateDashboardOrderStatus } from "@/lib/api-client"
...
export function OrderManager({ initialOrders }: { initialOrders: DashboardOrder[] }) {
  ...
  const advance = async (order: DashboardOrder) => {
    const next = getNextStatus(order.status)
    if (!next) return
    try {
      await updateDashboardOrderStatus(order.id, next)
      setOrders((prev) =>
  ...
  const cancel = async (order: DashboardOrder) => {
    try {
      await updateDashboardOrderStatus(order.id, "cancelled")
      setOrders((prev) =>
