...
function MapBounds({ shops }: { shops: Shop[] }) {
  ...
  useEffect(() => {
    const shopsWithLocation = shops.filter((s) => s.location?.lat && s.location?.lng)
    if (shopsWithLocation.length > 0) {
      const bounds = new LatLngBounds(
        shopsWithLocation.map((shop) => [shop.location!.lat, shop.location!.lng])
      )
...
export function ShopMap({ shops, selectedShopId, onShopSelect }: ShopMapProps) {
  ...
  return (
    ...
    >
      ...
      {shops.filter((s) => s.location?.lat && s.location?.lng).map((shop) => (
        <Marker
          key={shop.id}
